    ...
    const load = async () => {
      ...
      try {
        const [sumRes, absRes, perRes] = await Promise.all([
          api.get('/api/face/attendance/summary', { params: { day, person_type: 'CONTRACTOR' } }),
          api.get('/api/face/attendance/absentees', { params: { day, person_type: 'CONTRACTOR' } }),
          api.get('/api/face/persons', { params: { person_type: 'CONTRACTOR', limit: 500 } }),
        ])
        setPresentRows(sumRes.data?.rows || [])
        setAbsentRows(absRes.data?.items || [])
        setAllPersons(perRes.data?.items || [])
      } catch (e) {
        console.error("Failed to load contractor attendance:", e)
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="space-y-4 text-slate-900">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">Contractor Workforce Attendance</h2>
            <p className="text-xs text-slate-500">Registered contractors, present workers, early exits, and absentees.</p>
          </div>
        </div>
