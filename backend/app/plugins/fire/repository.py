"""
Fire zone storage, the snapshot the plugin matches against, and the event
writer.
...
OVERRIDE_COLUMNS = ("min_score", "min_frames", "confirm_sec", "min_area_frac",
                    "clear_sec", "alert_cooldown_sec")


def new_zone_id() -> str:
    return f"FZ-{uuid.uuid4().hex[:10].upper()}"


def new_event_id() -> str:
    return f"FEV-{uuid.uuid4().hex[:12].upper()}"


class FireZoneRepository:
    def __init__(self, db):
        self.db = db

    # ---------------- zones ----------------
    def list_zones(self, camera_id: Optional[str] = None,
                   active_only: bool = False) -> List[FireZone]:
        q = self.db.query(FireZone)
        if camera_id:
            q = q.filter(FireZone.camera_id == camera_id)
        if active_only:
            q = q.filter(FireZone.is_active.is_(True))
        return q.order_by(FireZone.camera_id, FireZone.name).all()

    def get_zone(self, zone_id: str) -> Optional[FireZone]:
        return self.db.query(FireZone).filter(FireZone.zone_id == zone_id).first()

    def name_taken(self, camera_id: str, name: str,
                   exclude_id: Optional[str] = None) -> bool:
        q = self.db.query(FireZone).filter(
            FireZone.camera_id == camera_id, FireZone.name == name)
        if exclude_id:
            q = q.filter(FireZone.zone_id != exclude_id)
        return q.count() > 0

    def create_zone(self, **fields) -> FireZone:
        row = FireZone(zone_id=new_zone_id(), **fields)
        self.db.add(row)
        self.db.commit()
        self.db.refresh(row)
        fire_zone_registry.invalidate()
        return row

    def update_zone(self, zone: FireZone, **fields) -> FireZone:
    ...
    def cameras_with_zones(self) -> List[str]:
        rows = self.db.query(FireZone.camera_id).filter(
            FireZone.is_active.is_(True)).distinct().all()
        return [r[0] for r in rows]

    # ---------------- events ----------------
    def list_events(self, *, camera_id=None, zone_id=None, event_type=None,
                    kind=None, severity=None, acknowledged=None, since=None,
                    until=None, limit=50, offset=0):
        q = self.db.query(FireEvent)
        if camera_id:
            q = q.filter(FireEvent.camera_id == camera_id)
        if zone_id:
            q = q.filter(FireEvent.zone_id == zone_id)
        if event_type:
            q = q.filter(FireEvent.event_type == event_type)
        if kind:
            q = q.filter(FireEvent.kind == kind)
        if severity:
            q = q.filter(FireEvent.severity == severity)
        if acknowledged is not None:
            q = q.filter(FireEvent.acknowledged.is_(bool(acknowledged)))
        if since:
            q = q.filter(FireEvent.timestamp >= since)
        if until:
            q = q.filter(FireEvent.timestamp <= until)
        total = q.count()
        rows = (q.order_by(FireEvent.timestamp.desc())
                .offset(max(0, offset)).limit(max(1, min(500, limit))).all())
        return rows, total

    def get_event(self, event_id: str) -> Optional[FireEvent]:
        return self.db.query(FireEvent).filter(
            FireEvent.event_id == event_id).first()

    def acknowledge(self, event_id: str, who: Optional[str],
                    note: Optional[str] = None) -> Optional[FireEvent]:
    ...
    def summary(self, since: Optional[datetime] = None) -> dict:
        ...
        base = self.db.query(FireEvent).filter(FireEvent.timestamp >= since)
        by_kind = dict(
            self.db.query(FireEvent.kind, sqlfunc.count(FireEvent.event_id))
            .filter(FireEvent.timestamp >= since)
            .group_by(FireEvent.kind).all())
        by_camera = [
            {"camera_id": c, "camera_name": n, "count": k}
            for c, n, k in
            self.db.query(FireEvent.camera_id, FireEvent.camera_name,
                          sqlfunc.count(FireEvent.event_id))
            .filter(FireEvent.timestamp >= since)
            .group_by(FireEvent.camera_id, FireEvent.camera_name)
            .order_by(sqlfunc.count(FireEvent.event_id).desc()).limit(20).all()]
        return {
            "since": since.isoformat(),
            "total": base.count(),
            "unacknowledged": base.filter(FireEvent.acknowledged.is_(False)).count(),
            "by_kind": by_kind,
            "by_camera": by_camera,
            "statutory_notice": rules.STATUTORY_NOTICE,
        }
...
def zone_to_snapshot(row: FireZone) -> Optional[dict]:
    """
    ...
    return {
        ...
        "watch": rules.sanitize_kinds(row.watch),
...
class FireZoneRegistry:
    ...
    def refresh(self, force: bool = False) -> None:
        ...
        try:
            ...
            try:
                ...
                for row in FireZoneRepository(db).list_zones(active_only=True):
                    snap = zone_to_snapshot(row)
                    if snap is None:
                        logger.warning(
                            f"Fire zone {row.zone_id} ({row.name}) has unusable "
            ...
            # reconfigure fire detection on every camera on the site.
            ...
            if (time.monotonic() - self._last_error_log) > 60.0:
                ...
                logger.error(f"Fire zone registry refresh failed: {exc}")
        finally:
            self._lock.release()
    ...
    def for_camera(self, camera_id: str) -> List[dict]:
        ...
        monitoring WITH ITS EXCLUSION ZONES MISSING — so the welding bay and
        the sodium lamp that an operator carefully excluded would start
        raising fire alarms, which is exactly the failure that gets a safety
        system muted.
...
fire_zone_registry = FireZoneRegistry()
...
class FireEventLog:
    """Queues fire events and writes them from one background thread."""
    ...
    def _ensure_worker(self) -> None:
        ...
        with self._lock:
            if self._started:
                return
            self._thread = threading.Thread(target=self._run, name="fire-event-log",
                                            daemon=True)
            self._thread.start()
            self._started = True
    ...
    def record(self, payload: dict) -> bool:
        ...
        try:
            ...
            if self.dropped in (1, 10, 100) or self.dropped % 500 == 0:
                logger.warning(f"Fire event queue full; dropped {self.dropped}")
            return False
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                logger.error(f"Fire event write failed ({len(batch)} rows): {exc}")
            finally:
                for _ in batch:
        ...
        try:
            try:
                for payload in batch:
                    db.add(FireEvent(event_id=new_event_id(), **payload))
                db.commit()
                self.written += len(batch)
                return
            except Exception as exc:
                # One malformed payload would otherwise take the other 49
                # confirmed fire incidents down with it. Retry row by row so a
                # single bad record loses only itself, and say which.
                db.rollback()
                logger.warning(
                    f"Fire event batch of {len(batch)} failed ({exc}); "
            ...
            for payload in batch:
                try:
                    ...
                    self.dropped += 1
                    logger.error(
                        f"Dropped unwritable fire event for camera "
